...
See: https://www.kaggle.com/happycube/grupo-bimbo-inventory-demand/log-means-in-python-48578-v2/code
"""
...
demand_prod = (df_train.groupby(['Producto_ID']))\
              ['log_demand'].mean().to_dict()
...
def process_product(product_id, sub):
    ...
    df_test_pca = df_test_subset[['Producto_ID', 'Cliente_ID',
                                  'Agencia_ID']]
    ...
    for z in zip(df_test_subset.id, df_test_p, df_test_l_pr,
                 df_test_l_pca):

        # Work in order of preference.  With straight dicts this is 
...
df_submit.to_csv('submissions/meantest3.csv')
